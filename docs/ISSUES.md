...
**PERF-A1: `timestep_kernel` 全局标量 `atomicMin` 导致全线程竞争** [HIGH]
...
**PERF-A2: `update_and_l2_kernel` 全局标量 `atomicAdd` 累计 L2** [HIGH]
`src/aero/cfd/gpu_update.cu:88`
与 A1 同模式。建议：block-level tree reduction + final reduce kernel。

**PERF-A3: `viscous_flux_kernel_atomic` 无 colored 路径，始终使用原子操作** [HIGH]
`src/aero/cfd/gpu_viscous.cu:221-231,311-313`
每个内面 8 次 `real_atomic_add`。建议：实现 colored 变体（如 `euler_residual_kernel_colored`）。

**PERF-A4: `wall_force_kernel` 无 colored 路径** [HIGH]
`src/aero/cfd/gpu_wall.cu:79-84,145-150`
墙面数量少但相邻面可能竞争同一 6 元素 `d_forces` 数组。建议：per-block partial force 共享内存规约。

**PERF-A5: `bj_limiter_kernel` 无 colored 路径，CAS 循环退化为自旋锁** [HIGH]
...
**PERF-B2: `rans_source_kernel` `*d_min_dt` 标量冗余全局读取** [MEDIUM]
...
**PERF-C1: `compute_rans_source_gpu` 无条件 `cudaDeviceSynchronize` + D2H 回读** [MEDIUM]
...
**PERF-D1: 每次迭代保守→原始转换冗余 2-4 遍** [HIGH]
...
**PERF-D2: `rebuild_faces` 两次调用 `compute_mesh_metrics`** [HIGH]
`mesh_metrics.cpp:283,298`
第二次全部重算（体积、质心、面法线、壁面距离），实际仅边界面变化。建议：仅重算受影响边界面的法线和面积。

**PERF-D3: 边界面匹配线性搜索** [HIGH]
...
**PERF-G4: `stream_comp` 创建但未使用** [HIGH]
`gpu_solver.cu:111-113`
所有 kernel 在默认流上启动。MPI halo 仅在 `stream_comm`。建议：计算均用 `stream_comp`，halo 与内部面计算并发。

**PERF-G5: `dnrm2_gpu` 不必要 D2H→H2D 往返** [MEDIUM]
`fgmres_gpu.cu:92-96`
标量回读到主机再拷贝回设备。建议：设备端保留结果，仅需主机值的调用者显式 D2H。

**PERF-G6: 粘性缓冲推迟到首次粘性迭代分配** [MEDIUM]
...
**PERF-H1: 非 CUDA 测试 .cpp 文件经 nvcc 编译** [HIGH]
...
**PERF-H6: CUDA 架构自动检测中断，仅构建 sm_75** [HIGH]
...
**PERF-I4: `missile_lib` 可分离编译属性设置重复** [LOW]
...
|------|------|--------|-----|------|------|
| A. GPU 原子竞争 | 6 | 1 | 0 | 0 | 7 |
| B. GPU 带宽/占用 | 1 | 1 | 1 | 1 | 4 |
| C. GPU 同步/启动 | 0 | 1 | 2 | 1 | 4 |
| D. CPU 重复工作 | 4 | 0 | 0 | 0 | 4 |
| E. CPU 内存分配 | 1 | 3 | 0 | 0 | 4 |
| F. CPU 算法/I/O | 0 | 2 | 2 | 0 | 4 |
| G. 数据流 | 4 | 3 | 2 | 1 | 10 |
| H. 构建—编译 | 4 | 2 | 0 | 0 | 6 |
| I. 构建—CMake | 0 | 2 | 2 | 0 | 4 |
| **总计** | **20** | **15** | **9** | **3** | **47** |
