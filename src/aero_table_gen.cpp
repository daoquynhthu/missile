    ...
    const AeroTableConfig& cfg)
{
    ...
    // 4. CFD override is intentionally disabled until the new solver is rebuilt.
    if (cfg.use_fvm && !conditions.empty()) {
        std::cerr << "[aero_table_gen] CFD override is disabled: solver rebuild is in progress\n";
        return false;
    }
